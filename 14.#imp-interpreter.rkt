...
;this imterpreter was written with continuation passing style and
;all of the function was transformed into 0-argument tail call
;function (the same as jump), its parameter becomes global
;register(variable) so that this interpreter can be implemented
;in a poor langugae that only support goto (besides, we'll use
